...
export default function App() {
  ...
  const renderMainApp = () => {
    const t = isDarkMode ? { bg: 'bg-slate-950', text: 'text-slate-100', textSub: 'text-slate-400', card: 'bg-slate-900/50', border: 'border-slate-800' } : { bg: 'bg-white', text: 'text-slate-900', textSub: 'text-slate-500', card: 'bg-slate-50', border: 'border-slate-200' };
      ...
      <div className={`flex flex-col h-full ${t.bg} ${t.text} transition-colors duration-700`}>
        ...
        <div className="p-6 border-t border-slate-800/30 flex items-center justify-between opacity-60">
          <div className="flex gap-4">
             ...
             <button onClick={() => { if(window.confirm('確定要清空嗎？')) setSchedule({}); }} className={`text-[10px] font-bold text-red-400 flex items-center gap-1`}><Trash2 size={12}/> 清空</button>
          </div>
